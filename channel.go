...
func (channel *Channel) Invite(start, end string) (code int) {
	...
	if d.Transport == "TCP" {
		...
		if channel.tcpPortIndex++; channel.tcpPortIndex+config.MediaPort >= config.MediaPortMax {
	...
	if response.StatusCode() == 200 {
		...
		publisher := &Publisher{
			Stream: &engine.Stream{
				...
				AutoCloseAfter: &serverConfig.AutoCloseAfter,
			},
		}
		...
		(*GetSipServer(d.Transport)).Send(ack)
